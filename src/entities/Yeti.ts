...
export class Yeti {
  ...
  // ---------------------------------------------------------------------------
  update(playerX: number, playerScreenY: number, delta: number): void {
    const dt = delta / 1000;
    this.elapsed += delta;
  ...
  // ---------------------------------------------------------------------------
  private drawYeti(gfx: Phaser.GameObjects.Graphics): void {
    // --- Drop shadow at feet ---
    ...
    gfx.fillPoints(
      [{ x: 14, y: -2 }, { x: 40, y: 14 }, { x: 32, y: 30 }, { x: 9, y: 14 }],
      true,
    );
    // Left arm
    gfx.fillPoints(
    ...
    gfx.fillPoints(
      [{ x: 22, y: 6 }, { x: 38, y: 16 }, { x: 32, y: 30 }, { x: 18, y: 18 }],
      true,
    );
    gfx.fillPoints(
      [{ x: -22, y: 6 }, { x: -38, y: 16 }, { x: -32, y: 30 }, { x: -18, y: 18 }],
